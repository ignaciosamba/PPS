...
int iniciar_UART(void)
{
   ...
   {
      TMR2H = -(SYSCLK/BAUDRATE/2);
      CKCON &= ~0x22;                  // T1M = 1; SCA1:0 = xx
      CKCON |=  0x08;
   } 
   else if (SYSCLK/BAUDRATE/2/256 < 4) 
   {
      TMR2H = -(SYSCLK/BAUDRATE/2/4);
      CKCON &= ~0x22;                  // T1M = 0; SCA1:0 = 01                  
      CKCON |=  0x01;
   } 
   else if (SYSCLK/BAUDRATE/2/256 < 12) 
   {
      TMR2H = -(SYSCLK/BAUDRATE/2/12);
      CKCON &= ~0x22;                  // T1M = 0; SCA1:0 = 00
   } 
   else 
   {
      TMR2H = -(SYSCLK/BAUDRATE/2/48);
      CKCON &= ~0x22;                  // T1M = 0; SCA1:0 = 10
      CKCON |=  0x02;
   }

   TMR2L = TMR2H;                          // Init Timer1
